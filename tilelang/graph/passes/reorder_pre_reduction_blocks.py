...
def _is_reduction_block(block: tir.Block) -> bool:
...
def _block_writes(block: tir.Block) -> set[tir.buffer.Buffer]:
    """Return the set of buffers written by *block*."""
    return {r.buffer for r in block.writes}


def _block_reads(block: tir.Block) -> set[tir.buffer.Buffer]:
    """Return the set of buffers read by *block*."""
    return {r.buffer for r in block.reads}


def _stripped_body(stmt: tir.Stmt) -> Optional[tir.SeqStmt]:
...
def reorder_pre_reduction_blocks(mod):
    ...
    for gv, func in mod.functions_items():
        if not isinstance(func, tir.PrimFunc):
            continue
        if func.attrs and func.attrs.get("tir.is_scheduled", False):
            continue
...
def _reorder_one(func: tir.PrimFunc) -> Optional[tir.PrimFunc]:
    ...
    if not isinstance(root_block, tir.Block):
        return None
    ...
    for info in infos:
        ...
        earliest_consumer: Optional[int] = None
        for buf in info.writes:
            for other in infos:
                if other.idx <= red_idx:
                    continue
                if buf in other.reads:
                    if earliest_consumer is None or other.idx < earliest_consumer:
...
def _block_from_stmt(stmt: tir.Stmt) -> Optional[tir.Block]:
    """Extract the ``tir.Block`` from a ``For → BlockRealize → Block`` chain."""
    s = stmt
